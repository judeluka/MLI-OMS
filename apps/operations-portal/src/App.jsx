...
import SalesGridPage from './pages/SalesGridPage';
...
const GenericProtectedRoute = ({ isAuthenticated, onLogout, children }) => {
    if (!isAuthenticated) {
        return <Navigate to="/login" replace />;
    }
    return React.cloneElement(children, { onLogout }); // Pass onLogout to the child component
};
    ...
    <BrowserRouter>
      <Routes>
        ...
        >
          ...
          <Route path="centres" element={<CentresPage />} /> {/* Add Centres route */}
          ...
          <Route path="participants/:participantId/edit" element={<ParticipantFormPage />} />
          ...
          {/* Note: Sales Grid route moved to full-width layout below */}
        </Route>
        ...
        <Route
          ...
          element={
            <GenericProtectedRoute isAuthenticated={isAuthenticated} onLogout={handleLogout}>
              <SchedulerPage />
            </GenericProtectedRoute>
          }
        />

        {/* Sales Grid Route - Protected, but uses its own full-width layout */}
        <Route
          path="/sales-grid"
          element={
            <GenericProtectedRoute isAuthenticated={isAuthenticated} onLogout={handleLogout}>
