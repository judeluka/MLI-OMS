...
app.get('/api/groups', async (req, res) => {
  // Updated get all groups logic - removed flight columns, now includes flight arrays
    ...
    try {
        client = await pool.connect();
        // Get basic group information with agency name from join
    ...
    try {
        ...
        res.status(201).json({
            ...
            group: {
                ...
                agency: agency, // Return the agency name as provided
...
app.get('/api/sales-grid-groups', async (req, res) => {
    ...
    try {
        ...
        if (agencyId && agencyId !== 'all') {
            ...
            paramCount++;
            whereClause += ` AND LOWER(a.name) = LOWER($${paramCount})`;
...
app.get('/api/agencies', async (req, res) => {
    ...
    try {
        ...
        const agencies = result.rows.map(row => ({
            name: row.name,
            value: row.name // Use name as value for filtering
        }));
...
app.get('/api/centres', async (req, res) => {
    ...
    try {
        ...
        // Try to fetch from dedicated centres table first
        let query = `
        ...
        try {
            ...
            // If centres table doesn't exist, fall back to getting centres from groups table
            console.log('Centres table not found, falling back to groups table');
            ...
            const centres = result.rows.map((row, index) => ({
                id: index + 1, // Generate temporary IDs
                name: row.centre,
                location_address: null, // No address available from groups table
                value: row.centre
            }));
...
app.post('/api/centres', async (req, res) => {
    ...
    try {
        ...
        // Check if centres table exists, if not create it
        ...
        if (!tableExists.rows[0].exists) {
            ...
            console.log('Created centres table');
        }

        const insertQuery = `
            INSERT INTO centres (name, location_address)
            VALUES ($1, $2)
            RETURNING *
        `;

        const result = await client.query(insertQuery, [name.trim(), location_address || null]);
        ...
        if (err.code === '23505') { // Unique violation
            return res.status(409).json({ 
                success: false, 
                message: 'A centre with this name already exists.' 
            });
        }
...
// PUT /api/centres/:id - Update an existing centre
app.put('/api/centres/:id', async (req, res) => {
    ...
    try {
        ...
        const existsCheck = await client.query('SELECT id FROM centres WHERE id = $1', [parseInt(id, 10)]);
        if (existsCheck.rows.length === 0) {
        ...
        const result = await client.query(updateQuery, [name.trim(), location_address || null, parseInt(id, 10)]);
...
app.delete('/api/centres/:id', async (req, res) => {
    ...
    try {
        ...
        // Check if centre exists and get details
        const existsCheck = await client.query('SELECT id, name FROM centres WHERE id = $1', [parseInt(id, 10)]);
        if (existsCheck.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ success: false, message: 'Centre not found.' });
        }

        const centre = existsCheck.rows[0];

        // Check if centre is being used by any groups
        const usageCheck = await client.query('SELECT COUNT(*) as count FROM groups WHERE LOWER(centre) = LOWER($1)', [centre.name]);
        ...
        if (usageCount > 0) {
            ...
            return res.status(400).json({ 
                success: false, 
                message: `Cannot delete centre "${centre.name}". It is currently being used by ${usageCount} group(s).` 
            });
        }

        // Delete the centre
        await client.query('DELETE FROM centres WHERE id = $1', [parseInt(id, 10)]);

        await client.query('COMMIT');

        res.status(200).json({
            success: true,
            message: `Centre "${centre.name}" deleted successfully.`
        });
    } catch (err) {
        if (client) await client.query('ROLLBACK');
    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                dietary_requirements, date_of_birth, contract_start_date, contract_end_date,
...
app.post('/api/staff', async (req, res) => {
    ...
    try {
        ...
        const params = [
            ...
            phone || null, role || null, dietary_requirements || null,
            date_of_birth || null, contract_start_date || null, contract_end_date || null
        ];
...
app.put('/api/staff/:id', async (req, res) => {
    ...
    try {
        ...
        const existsCheck = await client.query('SELECT id FROM staff WHERE id = $1', [parseInt(id, 10)]);
        if (existsCheck.rows.length === 0) {
        ...
        const updateQuery = `
            UPDATE staff SET
                ...
                role = $5, dietary_requirements = $6, date_of_birth = $7,
                contract_start_date = $8, contract_end_date = $9, updated_at = CURRENT_TIMESTAMP
            WHERE id = $10
            RETURNING *
        `;
...
app.delete('/api/staff/:id', async (req, res) => {
    ...
    try {
        ...
        const existsCheck = await client.query('SELECT id, first_name, last_name FROM staff WHERE id = $1', [parseInt(id, 10)]);
        if (existsCheck.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ success: false, message: 'Staff member not found.' });
        }

        const staff = existsCheck.rows[0];

        const usageCheck = await client.query('SELECT COUNT(*) as count FROM accommodation_assignments WHERE staff_id = $1', [parseInt(id, 10)]);
        const usageCount = parseInt(usageCheck.rows[0].count, 10);
        ...
        if (usageCount > 0) {
            ...
            return res.status(400).json({
                success: false,
                message: `Cannot delete staff member "${staff.first_name} ${staff.last_name}". They have ${usageCount} accommodation assignment(s).`
            });
        }

        await client.query('DELETE FROM staff WHERE id = $1', [parseInt(id, 10)]);
        await client.query('COMMIT');
...
// GET /api/transport-transfers - Fetch all transfers with filters  
app.get('/api/transport-transfers', async (req, res) => {
    ...
    try {
        ...
        // Check if transfers table exists with transport schema, if not create it
        const tableCheckQuery = `
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
        ...
        if (!tableExists.rows[0].exists) {
            // Create the transfers table with transport-specific fields
            const createTableQuery = `
                CREATE TABLE transfers (
                    id SERIAL PRIMARY KEY,
                    transfer_type VARCHAR(20) NOT NULL CHECK (transfer_type IN ('airport', 'intercity', 'local')),
            ...
            `;
            await client.query(createTableQuery);
            console.log('Created transfers table with transport schema');
        } else {
            // Check if table has the right columns and add missing ones if needed
            ...
            const requiredColumns = [
                { name: 'transfer_type', sql: 'ALTER TABLE transfers ADD COLUMN transfer_type VARCHAR(20) CHECK (transfer_type IN (\'airport\', \'intercity\', \'local\'))' },
                { name: 'status', sql: 'ALTER TABLE transfers ADD COLUMN status VARCHAR(20) DEFAULT \'scheduled\' CHECK (status IN (\'scheduled\', \'in_progress\', \'completed\', \'cancelled\'))' },
                { name: 'from_location', sql: 'ALTER TABLE transfers ADD COLUMN from_location VARCHAR(255)' },
                { name: 'to_location', sql: 'ALTER TABLE transfers ADD COLUMN to_location VARCHAR(255)' },
                { name: 'vehicle_info', sql: 'ALTER TABLE transfers ADD COLUMN vehicle_info VARCHAR(255)' },
                { name: 'driver_info', sql: 'ALTER TABLE transfers ADD COLUMN driver_info VARCHAR(255)' }
            ];
            ...
            for (const column of requiredColumns) {
                if (!existingColumns.includes(column.name)) {
                    try {
                        ...
                        console.log(`Added column ${column.name} to transfers table`);
                    } catch (err) {
                        console.log(`Column ${column.name} might already exist or constraint failed:`, err.message);
                    }
                }
            }
        }
        ...
        if (type && ['airport', 'intercity', 'local'].includes(type)) {
            paramCount++;
            whereConditions.push(`transfer_type = $${paramCount}`);
            queryParams.push(type);
        }
        ...
        if (date_from) {
            paramCount++;
            whereConditions.push(`transfer_date >= $${paramCount}`);
        ...
        const query = `
            SELECT 
                id, transfer_type, transfer_date, transfer_time, status,
                from_location, to_location, vehicle_info, driver_info,
                capacity, notes, cost, created_at
            FROM transfers
            ${whereClause}
...
// === ACCOMMODATION ASSIGNMENTS ENDPOINTS ===

// GET /api/accommodation-assignments - Fetch accommodation assignments with filters
app.get('/api/accommodation-assignments', async (req, res) => {
    const { centre_id, date_from, date_to, person_type } = req.query;
    
    let client;
    try {
        client = await pool.connect();
        
        // Check if accommodation_assignments table exists, if not create it
        const tableCheckQuery = `
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
        ...
        if (!tableExists.rows[0].exists) {
            // Create the accommodation_assignments table
            const createTableQuery = `
                CREATE TABLE accommodation_assignments (
                    ...
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    CHECK (
                        (participant_id IS NOT NULL AND staff_id IS NULL) OR 
                        (participant_id IS NULL AND staff_id IS NOT NULL)
                    )
                );
            `;
            await client.query(createTableQuery);
            console.log('Created accommodation_assignments table');
        }
        ...
        if (centre_id) {
            paramCount++;
            whereConditions.push(`aa.centre_id = $${paramCount}`);
            queryParams.push(parseInt(centre_id, 10));
        }

        if (person_type && ['participant', 'staff'].includes(person_type)) {
            if (person_type === 'participant') {
                whereConditions.push('aa.participant_id IS NOT NULL');
            } else {
                whereConditions.push('aa.staff_id IS NOT NULL');
            }
        }

        if (date_from) {
            paramCount++;
            whereConditions.push(`aa.check_out_date >= $${paramCount} OR aa.check_out_date IS NULL`);
            queryParams.push(date_from);
        }

        if (date_to) {
            paramCount++;
            whereConditions.push(`aa.check_in_date <= $${paramCount}`);
            queryParams.push(date_to);
        }
        ...
        const query = `
            SELECT 
                aa.id, aa.centre_id, aa.participant_id, aa.staff_id,
                aa.check_in_date, aa.check_out_date, aa.room_number,
                aa.accommodation_type, aa.notes, aa.created_at,
                c.name as centre_name, c.location_address as centre_address,
                p.first_name as participant_first_name, p.last_name as participant_last_name,
                s.first_name as staff_first_name, s.last_name as staff_last_name,
                CASE 
                    WHEN aa.participant_id IS NOT NULL THEN 'participant'
                    WHEN aa.staff_id IS NOT NULL THEN 'staff'
                    ELSE 'unknown'
                END as person_type
            FROM accommodation_assignments aa
            LEFT JOIN centres c ON aa.centre_id = c.id
            LEFT JOIN participants p ON aa.participant_id = p.id
            LEFT JOIN staff s ON aa.staff_id = s.id
            ${whereClause}
            ORDER BY aa.check_in_date DESC, aa.created_at DESC
        `;
...
// GET /api/staff/:id - Fetch individual staff member
app.get('/api/staff/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('Error fetching staff member:', err.stack);
...
// GET /api/staff/:id/accommodation-assignments - Fetch accommodation assignments for a specific staff member
app.get('/api/staff/:id/accommodation-assignments', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const query = `
            SELECT 
                aa.id, aa.centre_id, aa.start_date, aa.end_date,
                aa.room_type, aa.accommodation_type, aa.accommodation_name,
                aa.address, aa.meal_plan, aa.cost_per_night, aa.accommodation_grade,
                aa.booking_reference, aa.special_requirements, aa.notes, aa.created_at,
                c.name as centre_name, c.location_address as centre_address
            FROM accommodation_assignments aa
            LEFT JOIN centres c ON aa.centre_id = c.id
            WHERE aa.staff_id = $1 AND aa.is_staff_accommodation = true
...
// GET /api/staff/:id/work-assignments - Fetch work assignments for a specific staff member
app.get('/api/staff/:id/work-assignments', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        // Check if work_assignments table exists, if not create it
        const tableCheckQuery = `
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
        ...
        if (!tableExists.rows[0].exists) {
            // Create the work_assignments table
            const createTableQuery = `
                CREATE TABLE work_assignments (
                    id SERIAL PRIMARY KEY,
                    staff_id INTEGER REFERENCES staff(id) ON DELETE CASCADE,
                    centre_id INTEGER NOT NULL REFERENCES centres(id) ON DELETE RESTRICT,
            ...
            `;
            await client.query(createTableQuery);
            console.log('Created work_assignments table');
        }
        
        const query = `
            SELECT 
                wa.id, wa.centre_id, wa.start_date, wa.end_date,
                wa.is_active, wa.notes, wa.created_at,
                c.name as centre_name, c.location_address as centre_address
            FROM work_assignments wa
            JOIN centres c ON wa.centre_id = c.id
            WHERE wa.staff_id = $1
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch work assignments.' });
    } finally {
        if (client) client.release();
    }
});

// GET /api/staff/:id/documents - Fetch documents for a specific staff member
app.get('/api/staff/:id/documents', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        // Check if staff_documents table exists, if not create it
        const tableCheckQuery = `
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
        ...
        if (!tableExists.rows[0].exists) {
            // Create the staff_documents table
            const createTableQuery = `
            ...
            `;
            await client.query(createTableQuery);
            console.log('Created staff_documents table');
        }
        
        const query = `
            SELECT 
                id, staff_id, name, type, file_path, file_size, mime_type,
...
// POST /api/staff/:id/work-assignments - Create a new work assignment
app.post('/api/staff/:id/work-assignments', async (req, res) => {
    const { id } = req.params;
    const { centre_id, start_date, end_date, notes } = req.body;

    if (isNaN(parseInt(id, 10))) {
        return res.status(400).json({ success: false, message: 'Invalid staff ID.' });
    }

    if (!centre_id || !start_date) {
        return res.status(400).json({ 
            success: false, 
            message: 'Centre ID and start date are required.' 
        });
    }
    ...
    try {
        ...
        const staffCheck = await client.query('SELECT id FROM staff WHERE id = $1', [parseInt(id, 10)]);
        if (staffCheck.rows.length === 0) {
            return res.status(404).json({ success: false, message: 'Staff member not found.' });
        }

        // Check if centre exists
        const centreCheck = await client.query('SELECT id FROM centres WHERE id = $1', [parseInt(centre_id, 10)]);
        ...
        const insertQuery = `
            INSERT INTO work_assignments (
                staff_id, centre_id, start_date, end_date, notes
            )
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *
        `;
        ...
        console.error('Error creating work assignment:', err.stack);
        res.status(500).json({ success: false, message: 'Failed to create work assignment.' });
    } finally {
        if (client) client.release();
    }
});

// POST /api/staff/:id/accommodation-assignments - Create a new accommodation assignment
app.post('/api/staff/:id/accommodation-assignments', async (req, res) => {
    const { id } = req.params;
    const { 
        centre_id, start_date, end_date, accommodation_name, 
        address, room_type, meal_plan, cost_per_night, 
        accommodation_grade, booking_reference, special_requirements, notes 
    } = req.body;
    ...
    try {
        ...
        const insertQuery = `
            INSERT INTO accommodation_assignments (
                assignment_type, staff_id, centre_id, start_date, end_date, 
                accommodation_name, address, room_type, meal_plan, cost_per_night,
                accommodation_grade, is_staff_accommodation, booking_reference, 
        ...
        const params = [
            'staff_assignment',
            parseInt(id, 10),
            centre_id ? parseInt(centre_id, 10) : null,
            start_date,
            end_date,
            ...
            room_type || null,
            meal_plan || null,
            cost_per_night ? parseFloat(cost_per_night) : null,
        ...
        console.error('Error creating accommodation assignment:', err.stack);
        res.status(500).json({ success: false, message: 'Failed to create accommodation assignment.' });
    } finally {
        if (client) client.release();
    }
});
