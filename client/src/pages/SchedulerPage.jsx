...
const agencyColorThemes = [
    ...
    // Add more themes if you have many agencies
];
    ...
    const fetchAndProcessGroups = async () => {
      ...
      try {
        ...
        if (data.success && Array.isArray(data.groups)) {
          ...
          setAllGroupsFlat(globallySortedGroups);

          const finalAgencyGroupConfig = [];
          const agencyColorMap = new Map(); 
          let colorIndex = 0;

          if (globallySortedGroups.length > 0) {
              let currentAgencyName = null;
              let currentAgencyObject = null;

              globallySortedGroups.forEach(group => {
                  if (group.agencyName !== currentAgencyName) {
                      if (currentAgencyObject) {
                          finalAgencyGroupConfig.push(currentAgencyObject);
                      }
                      currentAgencyName = group.agencyName;
                      
                      if (!agencyColorMap.has(currentAgencyName)) {
                          agencyColorMap.set(currentAgencyName, agencyColorThemes[colorIndex % agencyColorThemes.length]);
                          colorIndex++;
                      }
                      const theme = agencyColorMap.get(currentAgencyName);
                  ...
                  currentAgencyObject.groups.push({
                      name: group.name,
                      id: group.id, 
              ...
              <tr className="bg-slate-100">
                ...
                {agencyGroupConfig.map(agency => (
                  <th
                    ...
                    // Corrected line: Removed the JSX comment and added a fallback for agency.agencyDisplayColor
