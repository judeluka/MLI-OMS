...
function IndividualFlightPage() {
  ...
  const fetchGroups = async (flightObj) => {
    // For demo: fetch all groups and filter client-side (replace with a backend filter for production)
    try {
      const res = await fetch('http://localhost:5000/api/groups');
      const data = await res.json();
      if (res.ok && data.success) {
        // Match by flight_code, flight_date, and time (arrival or departure)
        const code = flightObj.flight_code;
        const date = flightObj.flight_date;
        const time = flightObj.flight_time;
        const type = flightObj.flight_type;
        const matched = data.groups.filter(g => {
          if (type === 'arrival') {
            return (
              g.flightArrivalCode === code &&
              g.arrivalDate === date &&
              g.flightArrivalTime && g.flightArrivalTime.startsWith(time.slice(0,5))
            );
          } else {
            return (
              g.flightDepartureCode === code &&
              g.departureDate === date &&
              g.flightDepartureTime && g.flightDepartureTime.startsWith(time.slice(0,5))
            );
          }
        });
        setGroups(matched);
